...
|--------|-----------|------------|
| Identifier format | `is.workflow.actions.*` | Wrapped via `is.workflow.actions.appintentexecution` or direct bundle-style identifiers |
| Origin | Legacy Shortcuts (pre-iOS 16) | App Intents framework (iOS 16+) |
| Invocation | Direct identifier in action | Usually via `WFAppIntentExecutionAction`; some exported shortcuts use the AppIntent identifier directly as `WFWorkflowActionIdentifier` |
| Scope | Core shortcut actions | System integrations, deep links, app extensions |

## How to Invoke AppIntents

AppIntents are commonly invoked using the `WFAppIntentExecutionAction` wrapper:
...
Current exported shortcuts can also contain direct AppIntent identifiers such as `com.apple.mobilenotes.SharingExtension` or `com.apple.mobiletimer.CancelTimerIntent` as the action's `WFWorkflowActionIdentifier`. When reverse-engineering or cloning an exported shortcut, preserve the direct identifier and its parameters exactly instead of forcing it through `is.workflow.actions.appintentexecution`.

## Direct AppIntent Identifiers from Production Samples

These identifiers were observed in current Momo or Vector exported shortcuts. They are treated as authoritative examples of direct AppIntent-style actions. Parameter keys are sample-derived and not complete schemas.

| WFWorkflowActionIdentifier | Count | Samples | Descriptor identifier / bundle | Observed parameter keys |
|----------------------------|------:|---------|--------------------------------|-------------------------|
| `com.apple.ShortcutsActions.GetOrientationAction` | 1 | Momo | `GetOrientationAction / com.apple.ShortcutsActions` | `AppIntentDescriptor, UUID` |
| `com.apple.intelligenceplatform.IntelligencePlatform.IntelligencePlatformDataActionsAppIntentsExtension.CalculateAppUsageIntent` | 1 | Momo | `- / -` | `UUID, activityType, during, endTime, selectedDevice, startTime` |
| `com.apple.mobilenotes.AddFileAttachmentLinkAction` | 2 | Momo | `AddFileAttachmentLinkAction / com.apple.Notes` | `AppIntentDescriptor, OpenWhenRun, UUID, file, name, note` |
| `com.apple.mobilenotes.CreateChecklistItemLinkAction` | 1 | Momo | `CreateChecklistItemLinkAction / com.apple.Notes` | `AppIntentDescriptor, OpenWhenRun, UUID, name, noteEntity` |
| `com.apple.mobilenotes.CreateFolderLinkAction` | 1 | Momo | `CreateFolderLinkAction / com.apple.Notes` | `AppIntentDescriptor, OpenWhenRun, UUID, name` |
| `com.apple.mobilenotes.SharingExtension` | 6 | Momo, Vector | `CreateNoteLinkAction / com.apple.Notes, com.apple.mobilenotes` | `AppIntentDescriptor, OpenWhenRun, UUID, WFCreateNoteInput, WFNoteGroup, folder, name` |
| `com.apple.mobilephone.call` | 1 | Vector | `- / com.apple.InCallService` | `IntentAppDefinition, UUID, WFCallContact` |
| `com.apple.mobiletimer-framework.MobileTimerIntents.MTCreateAlarmIntent` | 1 | Vector | `CreateAlarmIntent / com.apple.mobiletimer` | `AppIntentDescriptor, UUID, dateComponents, name` |
| `com.apple.mobiletimer.CancelTimerIntent` | 1 | Momo | `CancelTimerIntent / com.apple.mobiletimer` | `AppIntentDescriptor, UUID` |
| `com.apple.mobiletimer.StartStopwatchIntent` | 1 | Momo | `StartStopwatchIntent / com.apple.mobiletimer` | `AppIntentDescriptor, UUID` |
| `com.apple.reminders.TTRCreateListAppIntent` | 1 | Momo | `TTRCreateListAppIntent / com.apple.reminders` | `AppIntentDescriptor, OpenWhenRun, UUID, name` |
| `com.bot.doubao.FlowAskMainBotAppIntent` | 1 | Momo | `FlowAskMainBotAppIntent / com.bot.doubao` | `AppIntentDescriptor, OpenWhenRun, UUID, shouldOpenDoubaoPopup, userInputText` |
| `dk.simonbs.Scriptable.RunScriptInlineIntent` | 2 | Momo | `- / -` | `ShowWhenRun, UUID, parameter, script, texts, urls` |
| `net.whatsapp.WhatsApp.send` | 1 | Vector | `- / -` | `UUID, WhatsAppContact` |
